defmodule LinearSDK.Inputs.RootIssueSort do
  @moduledoc ~S'''
  GraphQL input_object `RootIssueSort`.

  Issue root-issue sorting options.

  ## Summary

  - Kind: `input_object`


  ## Input Fields

  | Name | Type | Default | Deprecated | Description |
  | --- | --- | --- | --- | --- |
  | `nulls` | LinearSDK.Enums.PaginationNulls | `last` | No | Whether nulls should be sorted first or last |
  | `order` | LinearSDK.Enums.PaginationSortOrder | `n/a` | No | The order for the individual sort |
  | `sort` | LinearSDK.Inputs.IssueSortInput! | `n/a` | No | The sort to apply to the root issues |
  '''
end
