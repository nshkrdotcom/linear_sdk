<!-- Generated file. Do not edit by hand. -->

# CustomerStatusUpdateInput

_No description._

## Summary

- Kind: `input_object`


## Input Fields

| Name | Type | Default | Deprecated | Description |
| --- | --- | --- | --- | --- |
| `color` | [`String`](../scalars/string-scalar.md) | `n/a` | No | The UI color of the status as a HEX string. |
| `description` | [`String`](../scalars/string-scalar.md) | `n/a` | No | Description of the status. |
| `displayName` | [`String`](../scalars/string-scalar.md) | `n/a` | No | The display name of the status. |
| `name` | [`String`](../scalars/string-scalar.md) | `n/a` | No | The name of the status. |
| `position` | [`Float`](../scalars/float-scalar.md) | `n/a` | No | The position of the status in the workspace's customer flow. |
